...
const ProjectViewComponent = ({ projectId }) => {
    // Extract projects from the imported JSON file
    ...
    return (
        <>
        <Box sx={{ width: '99%', display: 'flex', flexDirection: 'column', gap: 2, backgroundColor: isSidebarDarkMode?'#111821':''}}>
            {/* Header Section */}
            <Box sx={{ height: '20vh', backgroundColor: isSidebarDarkMode?'#111821':'#fafafa', display: 'flex', flexDirection: 'column', gap: 2, padding: 4 }}>
                <Typography
                    variant="h1"
                    sx={{
                        fontWeight: 'bold',
